...
Ansel sends a structured editing snapshot to the local Python server. The
Python server forwards that snapshot into the Codex app server, receives a
structured plan, and returns a stable response envelope to Ansel.
...
- Ansel declares the writable control surface dynamically from the active image state
...
- `capabilityManifest.targets`: required writable controls declared by Ansel
