...
def test_mid_turn_render_happy_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "x-ansel-stage=1" in preview_url
...
def test_sequential_applies_update_preview_each_time(
    ...
    assert "x-ansel-stage=1" in preview_1
    ...
    assert "x-ansel-stage=2" in preview_2
...
def test_batched_apply_renders_after_each_operation(
    ...
    assert result["contentItems"][1]["type"] == "inputImage"
    assert "x-ansel-stage=2" in result["contentItems"][1]["imageUrl"]
