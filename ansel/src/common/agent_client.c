...
{
  const char *env_endpoint = g_getenv("ANSEL_AGENT_SERVER_URL");
...
static int32_t _chat_job_run(dt_job_t *job)
{
  ...
  const char *env_timeout = g_getenv("ANSEL_AGENT_SERVER_TIMEOUT_SECONDS");
  ...
  else
  {
    ...
    if(!delivery->result.has_response && !delivery->result.transport_error)
      delivery->result.transport_error = g_strdup(_("chat stream ended without a final response"));
                                                      ...
                                                      GError **error)
{
  ...
  if(!job)
  {
    g_set_error(error, _agent_client_error_quark(), DT_AGENT_CLIENT_ERROR_QUEUE_UNAVAILABLE,
                "%s", _("Ansel background jobs are not ready yet"));
