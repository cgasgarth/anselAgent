...
AUTORUN_PROMPT="${AUTORUN_PROMPT:-${ANSEL_AGENT_TEST_AUTORUN_PROMPT:-Increase exposure by exactly 0.7 EV.}}"
AUTORUN_QUIT_AFTER_MS="${AUTORUN_QUIT_AFTER_MS:-${ANSEL_AGENT_TEST_AUTORUN_QUIT_AFTER_MS:-1000}}"
...
HOST="$HOST" \
  ...
  ANSEL_AGENT_CODEX_TIMEOUT_SECONDS="$SERVER_TIMEOUT_SECONDS" \
...
if status != expected_status:
    raise SystemExit(
        f"Unexpected ansel status {status!r}, expected {expected_status!r}: "
        f"{result.get('error', '')}"
    )
